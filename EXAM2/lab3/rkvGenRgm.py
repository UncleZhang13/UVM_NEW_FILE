...
class GenRgm:
    ...
    def __init__(self):
        ...
        self._rgmItemList = []
        self._curRegName = ''
        self._uvmFileName = '.\mcdf_rgm_pkg_ref.sv'
        ...
        value_cg = new();\n\
                ...
                for fd in rgmObj._properties['fields']:
                    uvmFile.write('      %s.configure(this, %0d, %s, "%s", 0, \'h%s, 1, 0, 0);\n' % (fd['field'], int(
                        fd['bitpos_end'])-int(fd['bitpos_start'])+1, fd['bitpos_start'], fd['field_access'], fd['reset_value'].replace('0x', '')))
...
if __name__ == '__main__':
    ...
    #     print('%s \n[testReadClockFile] started \n%s\n' % (40*'*', 40*'*'))
    #     gen = GenRgm()
    #     gen.readRgmFile('mcdf_regs_v2.csv')
    #     gen.genUVMFile()
    #     print('%s \n[testReadClockFile] PASSED \n%s\n' % (40*'*', 40*'*'))
    # moduleTestList.append(testReadRgmFile)
